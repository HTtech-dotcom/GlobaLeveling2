...
import { getAgeGroup, getOccupationLabel, getRegionLabel } from "@/config/metrics";
...
export async function POST(request: NextRequest) {
  ...
  const occupation = String(body.occupation ?? auth.user.occupation ?? "general");
  ...
  await prisma.activityLog.create({
    data: {
      ...
      eventDetailJson: JSON.stringify({ age, gender, occupation, regionCode, currentPlanCode, trainingIntensity, theme })
    }
  });
