...
export async function POST(request: NextRequest) {
  ...
  const name = body.name == null ? auth.user.name : String(body.name).trim() || auth.user.name;
  ...
  await prisma.activityLog.create({
    data: {
      ...
      eventDetailJson: JSON.stringify({ name, age, gender, occupation, industryGroupCode: profession.industryGroupCode, regionCode, currentPlanCode, trainingIntensity, theme })
    }
  });
