    ...
    metrics.map(async (metric) => {
      const [baseline, age, region] = await Promise.all([
        prisma.userMetric.findMany({
          where: { metricDefinitionId: metric.metricDefinitionId, user: { status: "active" } },
          select: { score: true }
        }),
        prisma.userMetric.findMany({
          where: {
            metricDefinitionId: metric.metricDefinitionId,
            user: { status: "active", ageGroup: user.ageGroup }
          },
          select: { score: true }
        }),
        prisma.userMetric.findMany({
          where: {
            metricDefinitionId: metric.metricDefinitionId,
            user: { status: "active", regionCode: user.regionCode }
          },
          select: { score: true }
        })
      ]);
