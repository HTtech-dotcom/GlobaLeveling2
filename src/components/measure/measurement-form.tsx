...
const fieldMap: Record<string, FieldDefinition[]> = {
  ...
  HEA: [
    ...
    { name: "stepsPerDay", label: "Average steps per day", type: "number" },
    { name: "restingHr", label: "Resting heart rate", type: "number" }
  ]
};
...
type SaveResponse = {
  message?: string;
  score?: number;
  metrics?: Array<{ metricCode: string; metricName: string; score: number; rankCode?: string; confidenceStatus?: string; rawValue?: Record<string, unknown> | null }>;
...
function safeRawSignature(value: Record<string, unknown> | null | undefined) {
  try {
...
export function MeasurementForm({
  ...
  const savedRawValue = useMemo(() => {
    return appData.metrics.find((metric) => metric.metricCode === metricCode)?.rawValue ?? null;
  }, [appData.metrics, metricCode]);
  const savedRawSignature = useMemo(() => safeRawSignature(savedRawValue), [savedRawValue]);
  ...
  useEffect(() => {
    const saved = savedRawValue ?? {};

    if (metricCode === "CRR") {
      const defaults = buildJobDefaults(appData.user?.occupation, appData.user?.regionCode);
      const savedProfessionCode = normalizeProfessionCode(toFormValue(saved.professionCode) || defaults.professionCode);
      const professionDefaults = buildJobDefaults(savedProfessionCode, appData.user?.regionCode);
      const next: Record<string, string> = {
        ...professionDefaults,
        professionCode: savedProfessionCode
      };

      for (const key of Object.keys(professionDefaults)) {
      ...
      const availableCertifications = getCertificationTracksForProfession(savedProfessionCode);
      if (!availableCertifications.some((track) => track.code === next.certificationCode)) {
        next.certificationCode = availableCertifications[0]?.code ?? "none";
        next.certificationStage = availableCertifications[0]?.stages?.[0]?.code ?? "";
      }
    ...
    for (const field of fieldMap[metricCode] ?? []) {
      ...
      next[field.name] = savedValue === undefined || savedValue === null ? field.options?.[0]?.value ?? "" : toFormValue(savedValue);
    }

    setValues(next);
    setMessage("");
  }, [metricCode, savedRawSignature, savedRawValue, appData.user?.occupation, appData.user?.regionCode]);
