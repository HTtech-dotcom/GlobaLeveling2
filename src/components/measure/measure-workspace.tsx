...
import { genderOptions, metricCatalog } from "@/config/metrics";
    ...
    <div className="space-y-4">
      ...
      <section className="card space-y-3 p-4">
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          <label className="block space-y-1">
            <div className="text-sm text-muted">Gender baseline</div>
            <select
              className="input"
              value={profile.gender}
              onChange={(event) => setProfile((current) => ({ ...current, gender: event.target.value }))}
            >
              {genderOptions.map((item) => <option key={item.code} value={item.code}>{item.label}</option>)}
            </select>
          </label>
        ...
        {profileMessage ? <div className="message-text text-sm">{profileMessage}</div> : null}
      </section>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          {metricCatalog.map((metric) => {
            ...
            return (
              <button
                key={metric.code}
                className={`card-soft metric-tab p-4 text-left ${selectedMetric === metric.code ? "metric-tab-active" : ""}`}
