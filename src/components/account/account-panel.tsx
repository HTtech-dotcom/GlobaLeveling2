...
import { genderOptions } from "@/config/metrics";
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="card space-y-3 p-4">
        ...
        <select className="input" value={profile.gender} onChange={(event) => updateProfile({ gender: event.target.value })}>
      ...
      <section className="card p-4">
        ...
        {message ? <div className="message-text mt-3 text-sm">{message}</div> : null}
      </section>
    </div>
  );
}
