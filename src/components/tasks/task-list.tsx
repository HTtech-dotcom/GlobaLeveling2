...
import { CompletionToast, type CompletionToastState } from "./completion-toast";
import { safeJson } from "@/lib/safe-json";
import { useAppData } from "@/components/providers/app-data-provider";
import { getDisplayRank } from "@/config/rank-visual";
...
function rankBase(rank: string | undefined) {
  return String(rank ?? "").replace(/[+-]/g, "");
}
...
export function TaskList() {
  ...
  const activeTimedTaskId = useMemo(() => {
    return tasks.find((task) => isTimedTask(task) && task.status === "in_progress" && Boolean(task.timerStartedAt))?.id ?? null;
  }, [tasks]);

  const activeTimedTaskTitle = useMemo(() => {
    return tasks.find((task) => task.id === activeTimedTaskId)?.title ?? "another task";
  }, [activeTimedTaskId, tasks]);

  async function startTask(task: TaskItem) {
    if (!isTimedTask(task) || pendingTaskIds[task.id]) return;
    if (activeTimedTaskId && activeTimedTaskId !== task.id) return;
  ...
  async function completeTask(task: TaskItem) {
    ...
    const previousMetric = data.metrics.find((metric) => metric.metricCode === task.metricCode);
    const previousMetricRank = previousMetric ? getDisplayRank(previousMetric.score).label : undefined;
    const previousOverallRank = data.rank ? getDisplayRank(data.rank.overallScore).label : data.user ? getDisplayRank(data.user.currentOverallScore).label : undefined;
    setTaskPending(task.id, true);
    ...
    if (typeof payload?.metricCode === "string" && typeof payload?.reward === "number" && payload.reward > 0) {
      const nextMetrics = Array.isArray(payload.metrics) ? payload.metrics : data.metrics;
      const nextMetric = nextMetrics.find((metric: any) => metric.metricCode === payload.metricCode);
      const nextMetricRank = nextMetric && typeof nextMetric.score === "number" ? getDisplayRank(nextMetric.score).label : previousMetricRank;
      const nextRankSummary = Array.isArray(payload.metrics) ? calculateRank(payload.metrics) : payload.rank;
      const nextOverallScore = typeof nextRankSummary?.overallScore === "number" ? nextRankSummary.overallScore : data.rank?.overallScore ?? data.user?.currentOverallScore ?? 0;
      const nextOverallRank = getDisplayRank(nextOverallScore).label;

      const didMajorOverallRankUp = previousOverallRank && nextOverallRank && rankBase(previousOverallRank) !== rankBase(nextOverallRank);
      const eventType = didMajorOverallRankUp
        ? "overall_rank_up"
        : previousMetricRank && nextMetricRank && previousMetricRank !== nextMetricRank
          ? "skill_rank_up"
          : "task_reward";
  ...
  return (
    <>
      <CompletionToast toast={toast} gender={data.user?.gender ?? "male"} onClose={() => setToast(null)} />
      ...
      <div className="space-y-3">
        {visibleTasks.map((task) => {
          ...
          const isActiveTask = activeTimedTaskId === task.id;
          const isBlockedByActiveTimer = timed && task.status === "new" && Boolean(activeTimedTaskId) && activeTimedTaskId !== task.id;

          return (
            <div
              key={task.id}
              className={`card task-card p-4 ${isActiveTask ? "task-card-active" : ""} ${isBlockedByActiveTimer ? "task-card-blocked" : ""}`}
              onContextMenu={(event) => {
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {task.description && !task.workCheck ? <div className="mt-1 text-sm text-muted">{task.description}</div> : null}
                  {task.volumeMain ? <div className="task-volume-main mt-3 text-sm">{task.volumeMain}</div> : null}
                  ...
                  {timed ? (
                    <div className={`mt-3 inline-chip ${isActiveTask ? "active-timer-chip" : ""}`}>
                      {isActiveTask
                        ? `ACTIVE · Time left ${formatMsAsClock(remainingMs)}`
                        : task.status === "in_progress"
                          ? `Time left ${formatMsAsClock(remainingMs)}`
                          : `Timer ${formatMsAsClock(requiredMs)}`}
                    </div>
                  ) : null}
                  {isBlockedByActiveTimer ? <div className="mt-2 text-xs text-muted">Timer locked: {activeTimedTaskTitle} is running.</div> : null}
                </div>
                <div className={`status-chip rounded-full px-3 py-1 text-xs font-bold ${task.status === "completed" ? "status-chip-completed" : "status-chip-open"}`}>
              ...
              {timed && task.status === "new" ? (
                <button className="action-btn mt-4 w-full" disabled={syncing || isBlockedByActiveTimer} onClick={() => void startTask(task)} type="button">
                  {syncing ? "Starting..." : isBlockedByActiveTimer ? "Another task is running" : "Start"}
                </button>
